...
from models.symbols import ChunkNode, FileNode
...
def parse_imports(import_matches: list[int, dict], code: str) -> list[str]:
    ...
    for _, capture in import_matches:
        if "module" in capture:
            ...
            modules.append(code[node.start_byte: node.end_byte])
...
def parse_chunks(chunk_matches, exported_pos: set, code: str, file_node) -> list[ChunkNode]:
    seen = set()
    chunks = []
...
def extract(file_node: FileNode, tree, source_code):
    ...
    file_node.chunks = parse_chunks(raw.get("chunks", []), exported_positions, source_code, file_node)
